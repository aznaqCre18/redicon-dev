...
const HorizontalNavLink = (props: Props) => {
  ...
  // check permission
  if (item.permission && !auth.checkPermission(item.permission)) {
    return false
  }

  console.log('xdebug: module', item.module)

  if (item.module && !auth.checkModuleVendor(item.module)) {
    return false
  }
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <ListItemIcon sx={{ mr: 1, color: 'text.secondary' }}>
                <UserIcon
                  icon={icon}
