export function terbilang(num: number): string {
  const bilangan = terbilangCaps(num)

  return bilangan.charAt(0).toUpperCase() + bilangan.slice(1).toLocaleLowerCase()
}

export function terbilangCaps(num: number): string {
