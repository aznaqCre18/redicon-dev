...
const FormItemInvoice = ({
  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.id != undefined) {
        ...
        //   }
        // } else {
        console.log('debugx membershipLevel', membershipLevel)

        _price = product.variant_id
          ? product.product.variants.find(item => item.id == product.variant_id)?.price[
              membershipLevel
            ]
          : product.product.price[membershipLevel - 1].price

        console.log('debugx price', _price)

        // }
        setSku(product.product.product.sku)
        setSkuOld(product.product.product.sku)
        setPrice(_price)
      } else if (!product.priceCustom) {
        _price = product.variant_id
          ? product.product.variants.find(item => item.id == product.variant_id)?.price[
              membershipLevel
            ]
  ...
  }, [price, product, unit, membershipLevel])
  ...
  const handleSelectProduct = (value: ProductDetailType) => {
    ...
    if (discountType == 'percentage') {
      discount = (discount / 100) * value.price[membershipLevel - 1].price
    }
    ...
    setProductInvoice({
      qty: 1,
      total: value.variants ? 0 : value.price[membershipLevel - 1].price,
      discount: discount,
      price: value.variants ? 0 : value.price[membershipLevel - 1].price,
      product: value,
  ...
  useEffect(() => {
    if (price != null && price != product?.price && product?.priceCustom == false) {
      setProductInvoice({
        ...(product as any),
        price: price
      })
                ...
                onSelect={value => {
                  if (value) {
                    ...
                    if (existVariant) {
                      ...
                      setPrice(value.price[membershipLevel])

                      setProductInvoice({
                        ...product,
                        price: value.price[membershipLevel],
                        total: product.qty * value.price[membershipLevel],
                        variant_id: value.id
                      })
                    }
                  } else {
                ...
                <InputGroupComponent>
                  <Tooltip
                    title={
                      ...
                      // t('Stock') +
                      // ': ' +
                      // (product?.product.variants && product?.variant_id
                      //   ? product?.product.variants.find(item => item.id == product?.variant_id)
                      //       ?.stock ?? 0
...
export default FormItemInvoice
