...
import FormItemReturnInvoice, { InvoiceType } from './FormItemReturnInvoice'
...
const FormReturnInvoice = ({ data: dataOld }: { data?: ReturnSaleDetailType }) => {
  ...
  const create = useMutation(returnSaleService.create, {
    onSuccess: (data: any) => {
  ...
  const update = useMutation(returnSaleService.update, {
    onSuccess: (data: any) => {
      toast.success(t((data as unknown as ResponseType).data.message))
  ...
  const {
    ...
    handleSubmit,
    setValue
  } = useForm<ReturnSaleData | ReturnSaleDataUpdate>({
    mode: 'all',
    defaultValues: {
      payment_method: 'TUNAI',
      return_sale_number: ''
    },
    resolver: yupResolver(dataOld ? ReturnSaleUpdateSchema : ReturnSaleSchema)
  })

  const onSubmit = (dataNew: ReturnSaleData | ReturnSaleDataUpdate) => {
    ...
    if (dataOld) {
      const dataOldIds = (dataOld.return_sale_items ?? []).map(item => item.id)
      const dataNewUpdate = dataNew as ReturnSaleDataUpdate
      ...
      if (
        ...
        dataNewUpdate.return_sale_items.length == 0
      ) {
      ...
      update.mutate({
        ...
        data: dataNew as ReturnSaleDataUpdate
      })
    } else {
  ...
  return (
    <div>
      <CardWrapper>
        ...
        {errors.return_sale_items && (
          <Typography color='error'>
            {translateFormYupMsg(errors.return_sale_items.message)}
          </Typography>
        )}
...
export default FormReturnInvoice
