...
const CardChartDashboard = ({
  ...
  useEffect(() => {
    if (categoriesDate) {
      const penjualan = categoriesDate.map(c => {
        const found = data.find(
          d => formatDateFilter(new Date(d.order_date)) == formatDateFilter(c)
        )

        return found ? found.total_sales : 0
      })

      const PenjualanBruto = categoriesDate.map(c => {
        const found = data.find(
          d => formatDateFilter(new Date(d.order_date)) == formatDateFilter(c)
        )
      ...
      const pesanan = categoriesDate.map(c => {
        const found = data.find(
          d => formatDateFilter(new Date(d.order_date)) == formatDateFilter(c)
        )
      ...
      const produkTerjual = categoriesDate.map(c => {
        const found = data.find(
          d => formatDateFilter(new Date(d.order_date)) == formatDateFilter(c)
        )
      ...
      const pelanggan = categoriesDate.map(c => {
        const found = data.find(
          d => formatDateFilter(new Date(d.order_date)) == formatDateFilter(c)
        )
      ...
      const newOptions: ApexOptions = {
        ...
        xaxis: {
          ...
          categories: categoriesDate.map(c => `${c.getDate()}/${c.getMonth() + 1}`)
        }
      }
      ...
      console.log('setSeriesOptions', seriesOptions)
      console.log('setSeriesOptions', newSeries2)
      console.log('date', categoriesDate)
    }
      ...
      <CardContent>
        <Grid container spacing={2} mb={8} columns={13}>
          <Grid item xs={2}>
            <CardChartItem
              title={t('Sale') + ' (Neto)'}
              value={formatPriceIDR(data.reduce((acc, curr) => acc + curr.total_sales, 0))}
              color='success.main'
          ...
          <Grid item xs={2}>
            <CardChartItem
              title={t('Sale') + ' (Bruto)'}
              value={formatPriceIDR(data.reduce((acc, curr) => acc + curr.total_grand_sales, 0))}
              color='warning.main'
          ...
          <Grid item xs={2}>
            <CardChartItem
              title='Order'
              value={formatNumber(data.reduce((acc, curr) => acc + curr.total_orders, 0))}
              color='info.main'
          ...
          <Grid item xs={2}>
            <CardChartItem
              title='Product Sold'
              value={formatNumber(data.reduce((acc, curr) => acc + curr.total_product_sold, 0))}
              color='DarkOrchid'
          ...
          <Grid item xs={2}>
            <CardChartItem
              title='Customer'
              value={formatNumber(data.reduce((acc, curr) => acc + curr.total_customers, 0))}
              color='SlateBlue'
          ...
          <Grid item xs={3}>
            <CardChartItem
              ...
              value={formatPriceIDR(
                data.reduce((acc, curr) => acc + curr.sales_average, 0) / data.length,
                0
              )}
            />
          </Grid>
