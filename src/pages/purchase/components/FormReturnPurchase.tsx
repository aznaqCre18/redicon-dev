...
import {
  ...
  ReturnPurchaseUpdateSchema
} from 'src/types/apps/purchase/returnPurchase'

import { yupResolver } from '@hookform/resolvers/yup'
import { useRouter } from 'next/router'
import FormDialogSupplier from '../supplier/components/FormDialogSupplier'
...
const FormReturnPurchase = ({ data: dataOld }: { data?: ReturnPurchaseDetailType }) => {
  ...
  const create = useMutation(purchaseReturnService.create, {
    onSuccess: (data: any) => {
  ...
  const update = useMutation(purchaseReturnService.update, {
    onSuccess: (data: any) => {
      toast.success(t((data as unknown as ResponseType).data.message))
  ...
  const {
    ...
    handleSubmit,
    setValue
  } = useForm<ReturnPurchaseData | ReturnPurchaseDataUpdate>({
    mode: 'all',
    defaultValues: {
    ...
    resolver: yupResolver(dataOld ? ReturnPurchaseUpdateSchema : ReturnPurchaseSchema)
  })

  const onSubmit = (dataNew: ReturnPurchaseData | ReturnPurchaseDataUpdate) => {
    ...
    if (dataOld) {
      ...
      const dataNewUpdate = dataNew as ReturnPurchaseDataUpdate
  ...
  //   setValue('return_purchase_number', 'INV-' + Math.floor(Math.random() * 1000000000))
  // }
  ...
  useEffect(() => {
    if (dataOld) {
      ...
      promise(async () => {
        ...
        setValue('return_purchase_number', dataOld.return_purchase_number)
    ...
    <div>
      <CardWrapper>
                ...
                <Controller
                  ...
                  render={({ field: { value, ...field } }) => (
                    <TextField
                      ...
                      label={t('Return Purchase Number')}
                      // InputProps={{
                      //   endAdornment: (
        ...
        {errors.purchase_items && (
          <Typography color='error'>{errors.purchase_items.message}</Typography>
        )}
...
export default FormReturnPurchase
