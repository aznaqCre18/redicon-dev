...
const OrderItem = ({
      ...
      onSuccess: (data, variables) => {
        ...
        if (productData && productData.length > 0) {
          ...
          if (
            ...
            setPaginationData({ ...paginationData, query: '', page: 1 })
  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.id != undefined) {
        ...
        //   }
        // } else {
        _price = product.product.product.price[membershipLevel ?? 1]
        // }
        setSku(product.product.product.sku)
  ...
  const handleSelectProduct = (value: ProductDetailType) => {
    ...
    if (discountType == 'percentage') {
      discount = (discount / 100) * value.product.price[membershipLevel ?? 1]
    }
    ...
    console.log('xDebug, membership', membershipLevel)

    setProductInvoice({
      qty: 1,
      total: value.variants
        ? value.product.price[membershipLevel ?? 1]
        : value.product.price[membershipLevel ?? 1],
      discount: discount,
        ...
        {(showSelectProduct || product) && (
          <SelectCustom
            ...
            onInputChange={(_, value) => {
              if (value && value != '') onHandleSearch(value)
              else setPaginationData({ ...paginationData, query: '', page: 1 })
